...
void handleOneClient(BearSSL::WiFiClientSecure& client) {
  ...
  *sep = '\0';
  ...
  catalog::FetchResult fr = catalog::FetchResult::InvalidEnvelope;
  Serial.println(F("phase: TLS out (Catalog fetch)"));
  ...
  if (ok) {
    ...
    switch (fr) {
      case catalog::FetchResult::NotFound:
        code = 404;
        reason = "Not Found";
        json = "{\"error\":\"unknown_label\"}";
        break;
      case catalog::FetchResult::Unauthorized:
        code = 502;
        reason = "Bad Gateway";
        json = "{\"error\":\"catalog_auth\"}";
        break;
      case catalog::FetchResult::TlsError:
      case catalog::FetchResult::HttpError:
      case catalog::FetchResult::ServerError:
        code = 503;
        reason = "Service Unavailable";
        json = "{\"error\":\"catalog_unreachable\"}";
        break;
      case catalog::FetchResult::JsonError:
      case catalog::FetchResult::InvalidEnvelope:
        code = 502;
        reason = "Bad Gateway";
...
bool httpsPlayInit(const HttpsPlayConfig& cfg) {
  ...
  Serial.print(F("Emitter HTTPS server listening on port "));
