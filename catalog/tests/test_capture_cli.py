...
Maps to the manual capture CLI plan under ``plans/build/`` where automatable:
``commit`` + HTTP ``GET``, invalid/empty capture fails validation, preview summary.
...
def test_validate_rejects_empty_raw_us(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Invalid capture (e.g. empty) fails validate with an actionable message."""
    sd = tmp_path / "state"
...
def test_commit_visible_via_http_get(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Envelope committed via CLI round-trips through ``GET /v1/signals/{label}``."""
    sd = tmp_path / "state"
    db = tmp_path / "db.sqlite"
