...
Catalog exposes a versioned JSON **envelope** API backed by **SQLite (WAL)**. Authentication is a single shared **`IRPETE_API_KEY`** passed as `Authorization: Bearer <token>` on **every** `/v1` route, including **`GET /v1/health`**.

The **`irpete-capture`** CLI on the Raspberry Pi records TSOP pulse timings (RAM until `stop`), then **`validate`** / **`preview`** / **`commit`** using the **same** Pydantic rules and SQLite upsert as `POST /v1/signals`.

**HTTPS on the LAN** uses Uvicorn **`ssl_certfile` / `ssl_keyfile`** when **`IRPETE_TLS_CERTFILE`** and **`IRPETE_TLS_KEYFILE`** are set (paths to PEM files). Leave both unset for **local HTTP only** (`127.0.0.1`, default port **8000**).

A **systemd** unit template lets Catalog start **after the network is online**, **restarts on failure**, and log to **journald** (no secrets in the unit file—use **`/etc/irpete/catalog.env`**).
...
## Development run (HTTP, no TLS)

From this directory (`catalog/`):
...
The API listens on **`IRPETE_HOST` / `IRPETE_PORT`** (defaults **`127.0.0.1:8000`**). SQLite defaults to **`data/irpete.db`** under `catalog/` when **`IRPETE_DB_PATH`** is unset.

## HTTPS on the LAN (public DNS)
...
Operator curl/openssl examples use **`IRPETE_CATALOG_FQDN`**, **`IRPETE_EMITTER_FQDN`**, and optional **`IRPETE_LAN_IP`** from [`.env.example`](.env.example) (not read by the Python server).

### Prerequisites

- **DNS:** An **A** (and optionally **AAAA**) record for your Catalog FQDN (**`IRPETE_CATALOG_FQDN`**) pointing at the Raspberry Pi’s **LAN** address (split-horizon DNS is fine).
- **Certificates:** A wildcard or dedicated leaf whose **SAN** includes **`IRPETE_CATALOG_FQDN`**. Install **fullchain** + **private key** on the Pi (example paths: `/etc/irpete/fullchain.pem`, `/etc/irpete/privkey.pem`).
...
From `catalog/` after `pip install -e ".[dev]"` and loading secrets from `.env`:

```bash
./scripts/run-catalog-https.sh
```
...
From `catalog/` (or with the same vars in **`/etc/irpete/catalog.env`** on the Pi), load operator hostnames and secrets:

```bash
set -a && source .env && set +a
```

Replace **`IRPETE_API_KEY`** in `.env` if you have not already. When split-horizon DNS is not set up on the client, set **`IRPETE_LAN_IP`** to the Pi’s LAN address.
...
### CA PEM for Emitter firmware

ESP8266/BearSSL needs a **trust anchor**—usually the **issuing CA** PEM (not the leaf). Operators keep a copy as e.g. **`certs/catalog-ca.pem`** (gitignored); Emitter embeds bytes in **`secrets.h`** or SPIFFS later.

**If you already have the issuer CA file from your PKI**, copy it verbatim to **`catalog-ca.pem`**.
...
Document which PEM you embedded so rotations stay predictable: Emitter trusts the **CA**, so **leaf rotation** does not require a firmware change as long as the new leaf chains to the same anchor.

## systemd service (boot, restart, journald)

Catalog runs as a **system** service so it can bind **`IRPETE_PORT`** (recommended **8443**), read **`/etc/irpete/`** TLS material, and start without an interactive shell.

### Unit template and environment file

In the repo:

- [`deploy/systemd/irpete-catalog.service`](deploy/systemd/irpete-catalog.service) — copy to **`/etc/systemd/system/`** and adjust **`WorkingDirectory`** + **`ExecStart`** (venv **`python`** that has `pip install -e .`).
- [`deploy/catalog.env.example`](deploy/catalog.env.example) — keys to place in **`/etc/irpete/catalog.env`** (no committed secrets).

### Install on Raspberry Pi OS

1. Install the app and venv under a stable path (example **`/opt/irpete/catalog`**):

   ```bash
   sudo mkdir -p /opt/irpete
   sudo rsync -a ./catalog/ /opt/irpete/catalog/
   cd /opt/irpete/catalog
   sudo python3 -m venv .venv
   sudo .venv/bin/pip install -e .
   ```

2. Create **`/etc/irpete/catalog.env`** from **`deploy/catalog.env.example`**, set **`IRPETE_API_KEY`**, TLS paths, **`IRPETE_HOST=0.0.0.0`**, **`IRPETE_PORT=8443`**, and optionally **`IRPETE_DB_PATH`** (absolute path is best for reboot stability).

   ```bash
   sudo install -d -m 0755 /etc/irpete
   sudo install -m 0600 /path/to/your/catalog.env /etc/irpete/catalog.env
   sudo chown root:root /etc/irpete/catalog.env
   ```
...
3. Edit the copied unit so **`WorkingDirectory`** and **`ExecStart`** match step 1, then enable:
   ...
   ```bash
   sudo cp /opt/irpete/catalog/deploy/systemd/irpete-catalog.service /etc/systemd/system/
...
4. Confirm **`active (running)`**:
   ...
   ```bash
   systemctl status irpete-catalog --no-pager
   ```
...
```bash
journalctl -u irpete-catalog -b --no-pager
```
...
The unit is ordered **after** **`network-online.target`** so systemd waits for a configured “network is up” waiter. On **Wi‑Fi‑only** Pis, ensure your image enables **`systemd-networkd-wait-online.service`** or **`NetworkManager-wait-online.service`** as appropriate; otherwise DNS for **`IRPETE_CATALOG_FQDN`** may not be ready immediately. **Ethernet** is simpler for headless reliability.

### Cold reboot acceptance (operator)

After **`systemctl enable --now irpete-catalog`** works once:

1. **`sudo reboot`** (or power cycle).
2. Within **~1–3 minutes** (typical LAN + DHCP), from another host:

   ```bash
   set -a && source /etc/irpete/catalog.env && set +a
   curl -sS --resolve "${IRPETE_CATALOG_FQDN}:${IRPETE_PORT}:${IRPETE_LAN_IP}" \
     "https://${IRPETE_CATALOG_FQDN}:${IRPETE_PORT}/v1/health" \
     -H "Authorization: Bearer ${IRPETE_API_KEY}"
   ```
...
3. On the Pi, **`systemctl status irpete-catalog`** shows **active (running)**.
...
Within a few seconds, **`systemctl status irpete-catalog`** should show a fresh PID and **`journalctl -u irpete-catalog -n 20`** should show a new startup.
...
| **TLS / permission denied** | Key or cert path wrong, or PEM not readable by the service user; check **`journalctl -u irpete-catalog`** |
| **401 from health** | Missing **`IRPETE_API_KEY`** in **`/etc/irpete/catalog.env`** or wrong Bearer on the client |

## Manual IR capture CLI (`irpete-capture`)
...
Use one shell for Catalog’s API (optional, for `GET` verification) and another for capture, or commit offline and query later.

```bash
# Terminal A — HTTP API (local dev)
export IRPETE_API_KEY=dev-secret
