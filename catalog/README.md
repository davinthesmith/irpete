...
## HTTPS on the LAN (`catalog.toomanyprojects.dev`)
...
- **DNS:** An **A** (and optionally **AAAA**) record for **`catalog.toomanyprojects.dev`** pointing at the Raspberry Pi’s **LAN** address (split-horizon DNS is fine).
- **Certificates:** A wildcard **`*.toomanyprojects.dev`** or a dedicated leaf whose **SAN** includes **`catalog.toomanyprojects.dev`**. Install **fullchain** + **private key** on the Pi (example paths: `/etc/irpete/fullchain.pem`, `/etc/irpete/privkey.pem`).
...
```bash
curl -v --resolve catalog.toomanyprojects.dev:8443:192.168.x.x \
  "https://catalog.toomanyprojects.dev:8443/v1/health" \
...
```bash
openssl s_client -connect catalog.toomanyprojects.dev:8443 -servername catalog.toomanyprojects.dev </dev/null
```
...
The unit is ordered **after** **`network-online.target`** so systemd waits for a configured “network is up” waiter. On **Wi‑Fi‑only** Pis, ensure your image enables **`systemd-networkd-wait-online.service`** or **`NetworkManager-wait-online.service`** as appropriate; otherwise DNS for **`catalog.toomanyprojects.dev`** may not be ready immediately. **Ethernet** is simpler for headless reliability.
...
2. Within **~1–3 minutes** (typical LAN + DHCP), from another host:
   ...
   ```bash
   curl -sS --resolve catalog.toomanyprojects.dev:8443:192.168.x.x \
     "https://catalog.toomanyprojects.dev:8443/v1/health" \
